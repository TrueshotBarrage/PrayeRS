...
async function readData(altDataBinURL) {
  config = {
    headers: {
  ...
  dataBinURL = altDataBinURL || dataBinURL;
...
async function writeData(data, altDataBinURL) {
  const newData = JSON.stringify(data);
  dataBinURL = altDataBinURL || dataBinURL;
...
async function updateAux(isRider, req, res) {
  ...
  if (rideLocations.includes(location)) {
    ...
    readUsername(userId).then(async (username) => {
      ...
      await writeData(data).then((response) => {
